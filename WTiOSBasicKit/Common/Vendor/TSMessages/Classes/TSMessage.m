...
#define kTSMessageExtraDisplayTimePerPixel 0.01
#define kTSMessageAnimationDuration 0.1

#define kTSMessageVerticalOffset 0
...
//        verticalOffset += offset;
        verticalOffset += kTSMessageVerticalOffset;
    };
    ...
    if ([currentView.viewController isKindOfClass:[UINavigationController class]] || [currentView.viewController.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        if (![currentNavigationController isNavigationBarHidden])
        {
            ...
            verticalOffset = 0;//[currentNavigationController navigationBar].bounds.size.height;
    ...
    if (currentView.messagePosition == TSMessageNotificationPositionTop)
    {
        ...
        toPoint = CGPointMake(currentView.center.x,
                              navigationbarBottomOfViewController + verticalOffset + CGRectGetHeight(currentView.frame) / 2.0 - 2);
    }
    else
    {
    ...
    NSLog(@"message view = %@", NSStringFromCGRect(currentView.frame));
...
- (void)fadeOutNotification:(TSMessageView *)currentView
{
    ...
    if (currentView.messagePosition == TSMessageNotificationPositionTop)
    {
        fadeOutToPoint = CGPointMake(currentView.center.x, -CGRectGetHeight(currentView.frame)/2.f);//currentView.center.y);//
    }
    else
    {
